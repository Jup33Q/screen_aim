...
final class ScreenSampler: NSObject, SCStreamOutput {
    ...
    private func tickFPS() {
        ...
        if elapsed >= 2.0 {
            print(String(format: "FPS: %.1f", Double(frameCount) / elapsed))
            frameCount = 0
            fpsWindowStart = now
        }
    }
...
final class Calibrator: NSObject {
    ...
    func logLocalAim(markers n: Int, x: Double?, y: Double?) {
        ...
        if csvHandle == nil {
            ...
            if !FileManager.default.fileExists(atPath: url.path) {
                FileManager.default.createFile(atPath: url.path,
                                               contents: "timestamp,markers,x,y\n".data(using: .utf8))
            }
        ...
        let row = String(format: "%.3f,%d,%@\n", Date().timeIntervalSince1970, n, xy)
    ...
    func run() -> Never {
        ...
        if let content = win.contentView {
            for (id, c) in centers.enumerated() {
                ...
                guard let png = OpenCVBridge.markerPNG(withId: Int32(id), sidePixels: px),
                      let img = NSImage(data: png) else { continue }
            ...
            do {
                ...
                server.handshakePayload = {
                    ...
                    let payload: [String: Any] = [
                        "type": "calib",
                        "screenW": Double(W), "screenH": Double(H),
                        "markers": markers,
                    ]
                ...
                // Mac 端配对码开关 + 退出应用按钮：主覆盖层点击穿透，所以单独用一个可点击的悬浮 NSPanel（屏幕顶部居中）
                let qrBtnW: CGFloat = 52, closeBtnW: CGFloat = 52, btnGap: CGFloat = 8
                let panelH: CGFloat = 44
                let panelW = qrBtnW + closeBtnW + btnGap
                ...
                // 手机端控制消息（protocol.md §7）：配对二维码开关 / 本机识别结果上报
                server.onControl = { [weak self] msg in
                    ...
                    case "localAim":
                        // DEBUG: 手机本机识别结果（2Hz），与 Mac 端管线输出对照验证一致性；
                        // 全部上报追加到 scenes/localaim_*.csv 供离线分析识别质量
                        let n = msg["markers"] as? Int ?? 0
                        if let x = msg["x"] as? Double, let y = msg["y"] as? Double {
                            print(String(format: "LOCALAIM iPhone: screen=(%.1f, %.1f) markers=%d/4", x, y, n))
                            self?.logLocalAim(markers: n, x: x, y: y)
                            ...
                            print("LOCALAIM iPhone: 未集齐 4 角（检出 \(n)），无瞄准点")
                            self?.logLocalAim(markers: n, x: nil, y: nil)
                            DispatchQueue.main.async { [weak self] in
                                self?.debugLabel?.stringValue = "iPhone 瞄准: 未识别定位码（检出 \(n)/4）"
        ...
        print(String(format: "透明标定层已显示: 标记 %.0fpt (%.0f 物理像素) + %.0fpt 白卡，边距 %.0fpt，ESC 退出",
                     s, s * screen.backingScaleFactor, pad, m))
        app.run()
        exit(0)
    }
}
...
if CommandLine.arguments.contains("--calibrate") {
    ...
    Calibrator(markerSize: size, inset: inset, pad: pad, servePort: servePort).run()  // 阻塞
}
